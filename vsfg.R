...
#this assumes we are only interested in the part of the CCD chip which has above average signal
#you might wish to experiment with the criteria
#examines non-wavelength axis
columnsofinterest<-colSums(firstunpumped)>mean(colSums(firstunpumped));
print(paste("Light spread over",sum(columnsofinterest)/dim(firstunpumped)[2],"of the CCD is above threshold."));

#used only for wavelength integrated data
#find brightness threshold and pick out useful wavelengths
spectraofinterest<-rowSums(firstunpumped)>mean(rowSums(firstunpumped));
print(paste(sum(spectraofinterest)/dim(firstunpumped)[2],"of the wavelength axis is above threshold."));
...
for(j in 0:(scans-1)){
	...
	unpumpedfiles<-list.files(path=".",pattern=paste(file[1],".*",j,"s.*up.txt",sep=""));
	stopifnot(length(pumpedfiles)==length(unpumpedfiles));

		
	for( i in 1:length(pumpedfiles)){
		#read region of CCD data which contains signal on the non-wavelength axis
		pumpedspectrum<-rowSums(read.table(pumpedfiles[i])[,columnsofinterest]);
		unpumpedspectrum<-rowSums(read.table(unpumpedfiles[i])[,columnsofinterest]);
		
		#compute transmittance
		ratio<-pumpedspectrum/unpumpedspectrum;
...
allscan<-rbind(spectrumcal,allscan/scans);
...
write.table(cbind(
			...
			,colMeans(t(allscan[2:rows,spectraofinterest]),na.rm=TRUE) #data
